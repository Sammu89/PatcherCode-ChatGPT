...
class PatchApplier:
    ...
    def _apply_unified_hunk(self, content: List[str], hunk: Hunk, results: Dict[str, Any]) -> bool:
        ...
        # Find best position (ignore offsets)

        target_line = self._find_best_unified_position(content, hunk)

        if target_line is None:

            results['warnings'].append("Could not find position for unified hunk")
        ...
        if not self._validate_unified_context(content, hunk, target_line):
            ...
            results['warnings'].append(f"Context mismatch in unified hunk (line {target_line + 1})")
    ...
    def _apply_explicit_anchor_hunk(self, content: List[str], hunk: Hunk, results: Dict[str, Any], ui) -> bool:
        ...
        if not hunk.anchor:
            ...
            results['warnings'].append("Explicit anchor hunk with no defined anchor")
        ...
        if len(matches) == 0:
            ...
            results['warnings'].append(f"Anchor not found: '{hunk.anchor}'")
            ...
            if target_line is None:
                ...
                results['warnings'].append("Hunk application canceled by user")
    ...
    def _apply_implicit_anchor_hunk(self, content: List[str], hunk: Hunk, results: Dict[str, Any], ui) -> bool:
        ...
        if not hunk.anchor:
            ...
            results['warnings'].append("Implicit anchor hunk with no derived anchor")
        ...
        if len(matches) == 0:
            ...
            results['warnings'].append("Implicit anchor not found")
