...
class IndentationCorrector:
    ...
    def correct_file_indentation(self, content: str, file_path: Path) -> Tuple[str, List[str], bool]:
        ...
        if not self.is_python_file(file_path):
            ...
            return content, ["File is not Python - indentation not corrected"], False
        ...
        if not is_valid:
            ...
            warnings.append(f"Invalid syntax detected: {error_msg}")
        ...
        if uses_spaces != self.use_spaces:
            ...
            style_from = "spaces" if uses_spaces else "tabs"

            style_to = "spaces" if self.use_spaces else "tabs"

            warnings.append(f"Converting indentation from {style_from} to {style_to}")

        

        if uses_spaces and current_size != self.tab_size:

            warnings.append(f"Adjusting indentation size from {current_size} to {self.tab_size}")
        ...
        if was_modified:
            ...
            if not is_valid_final:
                ...
                warnings.append(f"WARNING: Correction may have introduced errors: {error_final}")

                # Em caso de erro, retornar original

                return content, warnings + ["Correction reverted due to errors"], False
    ...
    def get_correction_summary(self, analysis: Dict[str, any]) -> str:
        ...
        if analysis['has_tabs'] and analysis['has_spaces']:
            ...
            summary.append("⚠️  Mixed indentation detected (tabs and spaces)")
        ...
        if analysis['mixed_lines']:
            ...
            summary.append(f"⚠️  {count} line(s) with mixed tabs and spaces")

            

        if analysis['inconsistent_spacing']:

            summary.append("⚠️  Inconsistent indentation sizes")

            

        if not summary:

            summary.append("✅ Indentation consistent")

            

        summary.append(f"📊 {analysis['indented_lines']} indented lines of {analysis['total_lines']} total")
